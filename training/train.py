...
def train_model(model, train_loader, tokenizer, criterion, optimizer, device, epochs=10, accumulation_steps=4, start_epoch=0):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, (inputs, labels) in enumerate(progress_bar):
            ...
            writer.flush() 
        writer.add_scalar('Loss/train_epoch', running_loss / len(progress_bar), epoch)
        writer.add_scalar('Accuracy/Age_train_epoch', total_acc_age / len(progress_bar), epoch)
        writer.add_scalar('Accuracy/Gender_train_epoch', total_acc_gender / len(progress_bar), epoch)
        writer.flush()
